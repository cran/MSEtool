...
DFO_proj <- function(MSEobj,maxplot=6) {
  ...
  for(j in 1:length(plotorg)){
    ...
    for(i in plotorg[[j]]){
    DFO_Kobe(Br=MSEobj@SB_SBMSY[,i,proyears],Fr=MSEobj@F_FMSY[,i,proyears],xlab="",ylab="")
    ...
    mtext("SB/SBMSY",1,line=0.7,outer=T,font=2)
...
DFO_plot<-function(MSEobj,zero_origin=T){
  ...
  POFed<-apply(MSEobj@SB_SBMSY[,,yend] ,2,mean, na.rm = T)
...
DFO_bar<-function(MSEobj,yres=10){
  ...
  for(i in 1:nsec){
    ...
    POF[,,i]<-t(apply(MSEobj@SB_SBMSY[,,yind] ,2,quantile,p=c(0.05,0.25,0.5,0.75,0.95), na.rm = T))
  }
...
DFO_plot2 <- function(MSEobj, nam = NA,panel = T,Bcut=50, Ycut=50) {
  ...
  for (mm in 1:MSEobj@nMPs) {
    ...
    B50[mm] <- round(sum(MSEobj@SB_SBMSY[, mm, ] >= 0.5, na.rm = T)/prod(dim(MSEobj@SB_SBMSY[, mm, ])) * 100, 1)
...
DFO_quant<-function(MSEobj,maxcol=6,qcol=rgb(0.4,0.8,0.95), lcol= "dodgerblue4",curyr=2018,quants=c(0.05,0.25,0.75,0.95),addline=T, forreport=T){
  ...
  Yd<-MSEobj@Catch/ MSEobj@OM$RefY
...
DFO_tab<-function(MSEobj,maxMPs=15, rnd=0){
  ...
  P_Cr_S<-round(apply(MSEobj@SB_SBMSY[,,shortterm]<0.4,2,mean)*100,rnd)
  P_Ct_S<-round(apply(MSEobj@SB_SBMSY[,,shortterm]>0.4 & MSEobj@SB_SBMSY[,,shortterm]<0.8,2,mean)*100,rnd)
  P_H_S<-round(apply(MSEobj@SB_SBMSY[,,shortterm]>0.8,2,mean)*100,rnd)

  longterm<-max(1,MSEobj@proyears-10):MSEobj@proyears
  P_Cr_L<-round(apply(MSEobj@SB_SBMSY[,,longterm]<0.4,2,mean)*100,rnd)
  P_Ct_L<-round(apply(MSEobj@SB_SBMSY[,,longterm]>0.4 & MSEobj@SB_SBMSY[,,longterm]<0.8 ,2,mean)*100,rnd)
  P_H_L<-round(apply(MSEobj@SB_SBMSY[,,longterm]>0.8,2,mean)*100,rnd)

  refY<-MSEobj@OM$RefY
  STY<-round(apply(MSEobj@Catch[,,shortterm]/refY,2,mean)*100,rnd)
  LTY<-round(apply(MSEobj@Catch[,,longterm]/refY,2,mean)*100,rnd)
  ...
  AAVY <- round(apply((((MSEobj@Catch[, , y1] - MSEobj@Catch[, , y2])/MSEobj@Catch[, , y2])^2)^0.5, 2, mean, na.rm = T)*100,rnd)
