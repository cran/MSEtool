...
AAVY <- function(MSEobj=NULL, Ref=0.2, Yrs=NULL) {
 ...
 if (MSEobj@nMPs > 1) {
   ...
   AAVY <- array(apply(((((MSEobj@Catch[,1,y1]-MSEobj@Catch[,1,y2])/MSEobj@Catch[,1,y2])^2)^0.5),1,mean))
 }
...
AAVE <- function(MSEobj=NULL, Ref=0.2, Yrs=NULL) {
 ...
 if (MSEobj@nMPs > 1) {
   ...
   AAVE <- array(apply(((((MSEobj@Effort[,1,y1]-MSEobj@Effort[,1,y2])/MSEobj@Effort[,1,y2])^2)^0.5),1,mean))
 }
