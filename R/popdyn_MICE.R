...
popdynMICE<-function(qsx,qfracx,np,nf,nyears,nareas,maxage,Nx,VFx,FretAx,Effind,
  ...
  hsy<-ay <-by<-array(NA,c(np,nyears))
  hsy[,1]<-hsx; ay[,1]<-ax; by[,1]<-bx
  ...
  for(y in 2:(nyears+1)){
    ...
    if(y<=nyears){
      ...
      if (any(Linfarrayx[,y] != out$Linfx)) {
        Linfarrayx[,y] <- out$Linfx; gc <- TRUE
      }
      if (any(Karrayx[,y] != out$Kx)) {
        Karrayx[,y] <- out$Kx; gc <- TRUE
      } 
      if (any(t0arrayx[,y] != out$t0x)) {
        t0arrayx[,y] <- out$t0x; gc <- TRUE
      } 
      if (gc) {
        Len_age[,,y] <- out$Len_age
